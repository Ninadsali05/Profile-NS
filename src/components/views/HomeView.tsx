...
export const HomeView = () => {
  return (
    <div className="pb-20">
          ...
          <div className="flex flex-wrap justify-center items-center gap-8 py-8 border-y border-siemens-stone/50">
            {topSkills.map((skill, idx) => (
              <div key={skill} className="flex items-center gap-3">
                ...
                <span className="text-sm font-bold uppercase tracking-widest text-siemens-dark/60">{skill}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-6 mb-20">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl font-bold tracking-tighter text-siemens-dark mb-12 text-center uppercase">Experience Journey</h2>
          <div className="max-w-3xl mx-auto">
            <div className="relative border-l-2 border-siemens-petrol/20 ml-4 md:ml-0 space-y-16">
              ...
              >
                {/* Timeline Dot */}
                <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-siemens-dark border-4 border-siemens-light flex items-center justify-center">
                  <div className="w-1.5 h-1.5 rounded-full bg-siemens-petrol animate-pulse" />
                </div>

                <div className="p-8 rounded-sm border border-siemens-stone bg-white shadow-sm hover:border-siemens-petrol transition-all group">
                  <div className="flex items-center gap-2 text-xs font-bold text-siemens-petrol uppercase tracking-widest mb-2">
                    <Calendar className="w-3 h-3" />
                    {experiences[0].period}
                  </div>
                  <h3 className="text-2xl font-bold text-siemens-dark mb-1">{experiences[0].company}</h3>
                  <p className="text-siemens-petrol font-bold text-sm uppercase tracking-wider mb-4">{experiences[0].role}</p>
                  <div className="flex items-center gap-2 text-sm text-siemens-dark/40 mb-6">
                    <MapPin className="w-4 h-4" />
                    {experiences[0].location}
                  </div>
                  <p className="text-sm text-siemens-dark/70 leading-relaxed">
